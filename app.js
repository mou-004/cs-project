(() => {
  ...
  const ADMIN = {
    ...
    email: "admin@sentinelbank.mil",
    password: "Admin@123"
  };

  const EMPLOYEE = {
    name: "Bank Employee",
    email: "employee@sentinelbank.mil",
    password: "Employee@123"
  };

  const OPERATIONS = ["deposit", "withdrawal", "transfer"];

  // Existing balance already owned by the bank.
  // Change this value to set a different opening bank balance.
  const BANK_OPENING_VOLUME = 1000000;
  ...
  function calculateBankVolume() {
    const operations = getOperations();

    const customerTransactionVolume = operations.reduce(
      (total, operation) => {
    ...
    return roundMoney(
      BANK_OPENING_VOLUME + customerTransactionVolume
    );
  }
  ...
  function setSelectedRole(role) {
    ...
    $("#customerRegister").classList.toggle(
      "hidden",
      role !== "customer"
    );

    $("#customerVerificationActions").classList.toggle(
      "hidden",
      role !== "customer"
    );
  ...
  function openPanel(panelId) {
    ...
    if (panelId === "employeeSecurityPanel") {
      renderEmployeeVault();
    }

    if (panelId === "employeePanel") {
      renderEmployee();
    }

    if (panelId === "customerPanel") {
      renderCustomer();
    }
  }
  ...
  async function login(event) {
    ...
    if (!email || !password) {
      setMessage(
        $("#loginMsg"),
        "Enter the verified Customer email and password.",
        "error"
      );
    ...
    try {
      ...
      setMessage(
        $("#loginMsg"),
        "Checking verified Firebase Customer account…"
      );

      const customer =
        await window.FirebaseCustomerAuth.loginCustomer(
          email,
          password
        );
      ...
      if (!customer.emailVerified) {
        throw new Error(
          "Firebase did not confirm this email."
        );
      }

      const verifiedCustomers = customerList();

      const existing = verifiedCustomers.find(
      ...
      setMessage(
        $("#loginMsg"),
        window.FirebaseCustomerAuth.errorMessage?.(
          error
        ) ||
          error.message ||
  ...
  async function register(event) {
    ...
    try {
      ...
      setMessage(
        $("#registerMsg"),
        "Creating a pending account and sending the verification email…"
      );

      await window.FirebaseCustomerAuth.registerCustomer(
        name,
        email,
        password
      );
  ...
  async function resendCustomerVerification() {
    ...
    try {
      ...
      const result =
        await window.FirebaseCustomerAuth.resendVerification(
          email,
          password
        );
  ...
  function updateProfileFields(operation) {
    const isRailFence =
      $(`#${operation}Alg`).value === "railfence";

    $(`#${operation}KeyBox`).classList.toggle(
      "hidden",
      isRailFence
    );

    $(`#${operation}RailBox`).classList.toggle(
      "hidden",
      !isRailFence
    );
  }
  ...
  function clearProfileSetup(operation) {
    ...
    $(`#${operation}Cipher`).textContent =
      "Create an encrypted preview before locking.";
  }
  ...
  function renderProfile(operation) {
    const profile =
      getCustomerProfiles(currentUser.email)[operation];
  ...
  function previewSafety(operation) {
    try {
      ...
      setMessage(
        $(`#${operation}SafetyMsg`),
        "Ciphertext is ready. Lock this profile to hide every setup detail.",
        "success"
      );
    } catch (error) {
  ...
  function lockSafety(operation) {
    try {
      ...
      if (!preview) {
        throw new Error(
          "Create an encrypted preview first."
        );
      }
  ...
  function unlockProfileForEdit(operation) {
    ...
    try {
      if (!profile) {
        throw new Error(
          "No locked safety profile exists."
        );
      }
      ...
      if (!secret) {
        throw new Error(
          "Enter the private safety key."
        );
      }

      const plaintext = C().decrypt(
        profile.ciphertext,
        profile.algorithm,
        secret
      );
      ...
      if (
        hashText(plaintext) !== profile.plainHash
      ) {
        throw new Error(
          "Wrong key. The locked profile remains hidden."
        );
      }
      ...
      setMessage(
        $(`#${operation}SafetyMsg`),
        "Profile decrypted. You can now view or replace its private setup.",
        "success"
      );
    } catch (error) {
      setMessage(
  ...
  function renderCustomer({
    ...
    const rows = getOperations().filter(
      operation =>
        normalize(operation.customerEmail) ===
      ...
      ? rows
          .map(operation => `
            <tr>
              ...
              <td>
                ${money(operation.previousBalance)}
                →
                ${money(operation.newBalance)}
              </td>
              <td>
  ...
  function renderExecutionProfile() {
    ...
    $("#recipientLabel").classList.toggle(
      "hidden",
      operation !== "transfer"
    );

    $("#decryptionResultBox").classList.add(
      "hidden"
    );

    $("#decryptedOut").textContent = "";

    $("#balanceEquation").classList.add(
      "hidden"
    );
    ...
    if (!profile) {
      setState(
        ...
        `Create and lock ${titleCase(
          operation
        )} Safety before using this operation.`,
        "error"
      );
  ...
  function findAccount(reference) {
    ...
    if (all[normalizedReference]) {
      return all[normalizedReference];
    }

    return (
      Object.values(all).find(account => {
        return (
  ...
  function executeOperation(event) {
    ...
    try {
      ...
      if (!secret) {
        throw new Error(
          "Enter the private operation key."
        );
      }
      ...
      if (
        ...
        throw new Error(
          "Wrong key. Decryption failed and the balance was not changed."
        );
      }
      ...
      if (type === "deposit") {
        ...
        if (amount > previousBalance) {
          throw new Error(
            `Insufficient balance. Current balance: ${money(
              previousBalance
            )}.`
          );
        }
      ...
      $("#decryptionResultBox").classList.remove(
        "hidden"
      );

      $("#prevBalance").textContent =
        money(previousBalance);

      $("#opAmountShow").textContent =
        money(amount);
      ...
      $("#balanceEquation").classList.remove(
        "hidden"
      );
      ...
      setMessage(
        ...
        `${titleCase(
          type
        )} completed after successful private decryption.`,
        "success"
      );
  ...
  function renderAdmin() {
    ...
    // Opening bank balance + every customer transaction.
    $("#adminVolume").textContent =
      money(calculateBankVolume());

    $("#adminSafetyCount").textContent =
      Object.values(allProfiles).reduce(
        (total, profiles) =>
          total +
  ...
  function renderEmployee() {
    ...
    // Employee dashboard displays the same bank volume.
    $("#employeeVolume").textContent =
      money(calculateBankVolume());

    $("#employeeOps").innerHTML =
      operations.length
        ? operations
            .map(operation => `
              <tr>
  ...
  function updateChallengeFields() {
    ...
    $("#challengeKeyBox").classList.toggle(
      "hidden",
      railFence
    );

    $("#challengeRailBox").classList.toggle(
      "hidden",
      !railFence
    );
  }
  ...
  function clearChallengeSetup() {
    ...
    $("#challengeCipherPreview").textContent =
      "Create an encrypted preview before locking.";
  }
  ...
  function previewEmployeeVault() {
    try {
      ...
      setMessage(
        $("#challengeMsg"),
        "Ciphertext is ready. Lock the vault to hide all configuration details.",
        "success"
      );
    } catch (error) {
  ...
  function lockEmployeeVault() {
    try {
      ...
      saveEmployeeVault({
        ...
        attempts: 0,
        algorithm:
          employeePreview.algorithm,
        ciphertext:
          employeePreview.ciphertext,
        plainHash:
          employeePreview.plainHash,
        lockedAt: new Date().toISOString()
      });
  ...
  function unlockEmployeeVaultForEdit() {
    ...
    try {
      if (!vault?.configured) {
        throw new Error(
          "Employee vault is not configured."
        );
      }
      ...
      if (
        ...
        throw new Error(
          "Wrong key. Employee vault details remain hidden."
        );
      }
      ...
      setMessage(
        $("#challengeMsg"),
        "Employee vault decrypted. You can now view or replace its configuration.",
        "success"
      );
    } catch (error) {
      setMessage(
  ...
  function renderEmployeeVault() {
    ...
    $("#employeeVaultStatus").textContent =
      vault?.configured
        ? accessLocked
          ? "BLOCKED"
          : "LOCKED"
    ...
    $("#employeeAttemptMetric").textContent =
      `${attempts} / 3`;

    $("#employeeAccessMetric").textContent =
      enabled
        ? "ENABLED"
        : "DISABLED";
    ...
    if (!vault?.configured) {
      setState(
        ...
        "Create and lock an Employee access vault.",
        "error"
      );
      ...
      setState(
        $("#challengeStatus"),
        "ACCESS BLOCKED",
        "Three failed Employee decryptions were recorded.",
        "error"
      );
    } else if (!enabled) {
      setState(
        $("#challengeStatus"),
        "ACCESS DISABLED",
        "Employee login is disabled by the Administrator.",
        "error"
      );
    } else {
      setState(
        $("#challengeStatus"),
        "VAULT ACTIVE",
        "Employee access requires private decryption. No clue is displayed.",
        "success"
      );
    }

    $("#toggleEmployee").disabled =
      !vault?.configured;

    $("#resetEmployeeLock").disabled =
      !vault?.configured;

    $("#toggleEmployee").textContent =
      enabled
  ...
  function showEmployeeGate() {
    ...
    if (!vault?.configured) {
      ...
      setState(
        $("#gateState"),
        "ACCESS BLOCKED",
        "Administrator reset is required after three failed attempts.",
        "error"
      );
      ...
      setState(
        $("#gateState"),
        "ACCESS DISABLED",
        "Administrator has disabled Employee access.",
        "error"
      );
    } else {
      setState(
        $("#gateState"),
        "PRIVATE VAULT READY",
        `${vault.attempts || 0} of 3 failed attempts used.`,
        "success"
      );
    }
  }
  ...
  function unlockEmployee(event) {
    ...
    try {
      ...
      if (vault.accessLocked) {
        throw new Error(
          "Employee access is blocked. Administrator reset is required."
        );
      }
      ...
      if (!secret) {
        throw new Error(
          "Enter the private Employee vault key."
        );
      }
      ...
      if (
        ...
        vault.attempts =
          Number(vault.attempts || 0) + 1;

        if (vault.attempts >= 3) {
          vault.accessLocked = true;
        }
  ...
  function bindEvents() {
    ...
    OPERATIONS.forEach(operation => {
      $(`#${operation}Alg`).addEventListener(
        "change",
        () => {
          updateProfileFields(operation);
          invalidateProfilePreview(operation);
      ...
      $(`#${operation}Text`).addEventListener(
        "input",
        () =>
      ...
      $(`#${operation}Key`).addEventListener(
        "input",
        () =>
          invalidateProfilePreview(
            operation
      ...
      $(`#${operation}Rails`).addEventListener(
        "input",
        () =>
          invalidateProfilePreview(
            operation
    ...
    $$(".unlock-edit-btn").forEach(
      button => {
        button.addEventListener(
          "click",
          () =>
            unlockProfileForEdit(
              button.dataset.operation
            )
        );
      }
    ...
    $("#challengeText").addEventListener(
      "input",
      invalidateChallengePreview
    );

    $("#challengeKey").addEventListener(
      "input",
      invalidateChallengePreview
    );

    $("#challengeRails").addEventListener(
      "input",
      invalidateChallengePreview
    );
    ...
    $("#challengePreviewBtn").addEventListener(
      "click",
      previewEmployeeVault
    );

    $("#challengeLockBtn").addEventListener(
      "click",
      lockEmployeeVault
    );

    $("#challengeUnlockBtn").addEventListener(
      "click",
      unlockEmployeeVaultForEdit
    );

    $("#toggleEmployee").addEventListener(
      "click",
      toggleEmployeeAccess
    );

    $("#resetEmployeeLock").addEventListener(
      "click",
      resetEmployeeAttempts
    );
    ...
    $("#employeeRefresh").addEventListener(
      "click",
      renderEmployee
    );
  ...
  function initialize() {
    if (!C()) {
      setMessage(
        $("#loginMsg"),
        "Cipher algorithm module failed to load.",
        "error"
      );
